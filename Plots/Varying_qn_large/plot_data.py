...
def plot_seeds(ent_state_type, seeds, res_qd):
    ...
    for seed in seeds:
        ...
        for qn in qn_values:
            ...
            dec_val, p_idx = lsvm.get_desicion_values_for_data(f"Plots/Varying_qn_large/data_{ent_state_type}/measurements_werner_{ent_state_type}_res_{res_qd}_qn_{qn}_{seed}.npy",)
...
def plot_ent_seed(ent_state_type, seeds, res_qds):
    ...
    for i, res_qd in enumerate(res_qds):
        ...
        for seed in seeds:
            ...
            for qn in qns:
                lsvm = linear_svm(
                    f"Plots/Varying_qn_large/data_{ent_state_type}/measurements_train_res_{res_qd}_qn_{qn}_seed_{seed}.npy",
                    f"Plots/Varying_qn_large/data_{ent_state_type}/labels_train_res_{res_qd}_qn_{qn}_{seed}.npy",
                    f"Plots/Varying_qn_large/data_{ent_state_type}/measurements_test_res_{res_qd}_qn_{qn}_{seed}.npy",
                    f"Plots/Varying_qn_large/data_{ent_state_type}/labels_test_res_{res_qd}_qn_{qn}_{seed}.npy",
                    class_weights={-1: 1, 1: 5000},
                    C=1
                )
                accuracy, accuracy_sep, accuracy_ent, _ = lsvm.evaluate_model(evaluate_train=False)
