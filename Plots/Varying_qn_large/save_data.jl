...
# 
    ...
    save("Plots/Varying_qn_large/States/train_sep.jld2", "A", sep_states_train)
    ...
    save("Plots/Varying_qn_large/States/test_sep.jld2", "A", sep_states_test)
    ...
    #    save("Plots/Varying_qn_large/States/train_ent_$(ent_state_type).jld2", "A", ent_states_train)
         ent_states_test = generate_werner_states(d_main, nbr_ent_states, [ent_state_type], p_min)
    #    save("Plots/Varying_qn_large/States/test_ent_$(ent_state_type).jld2", "A", ent_states_test)
    end
#
    ##Define labels for the states
    labels_train = vcat([-1 for i in 1:size(ent_states_train)[1]], [1 for i in 1:size(sep_states_train)[1]])
    np.save("Plots/Varying_qn_large/States/train_labels.npy", labels_train)
    labels_test = vcat([-1 for i in 1:size(ent_states_train)[1]], [1 for i in 1:size(sep_states_test)[1]])
    np.save("Plots/Varying_qn_large/States/test_labels.npy", labels_test)
#
    ...
    for ent_state_type in ent_state_types
        ...
        save("Plots/Varying_qn_large/States/werner_$(ent_state_type).jld2", "A", ent_states)
    end
end
...
function save_data()
    ...
    sep_states_train = load("Plots/Varying_qn_large/States/train_sep.jld2")["A"]
    sep_states_test = load("Plots/Varying_qn_large/States/test_sep.jld2")["A"]

    for ent_state_type in ent_state_types
        ent_states_train = load("Plots/Varying_qn_large/States/train_ent_$(ent_state_type).jld2")["A"]
        ent_states_test = load("Plots/Varying_qn_large/States/test_ent_$(ent_state_type).jld2")["A"]
        werner_states = load("Plots/Varying_qn_large/States/werner_$(ent_state_type).jld2")["A"]
        for seed in seeds
            for res_qd in res_qds
                ...
                for qn in qns
                    ...
                    save("Plots/Varying_qn_large/Measurement_Operators/measurementop_res_$(res_qd)_qn_$(qn)_seed_$(seed).jld2", "A", eff_measurements)
                    ...
                    np.save("Plots/Varying_qn_large/data_$(ent_state_type)/measurements_train_$(ent_state_type)_res_$(res_qd)_qn_$(qn)_seed_$(seed).npy", measurements_train)                        
                    np.save("Plots/Varying_qn_large//data_$(ent_state_type)/measurements_test_$(ent_state_type)_res_$(res_qd)_qn_$(qn)_$(seed).npy", measurements_test)
                    np.save("Plots/Varying_qn_large//data_$(ent_state_type)/measurements_werner_$(ent_state_type)_res_$(res_qd)_qn_$(qn)_$(seed).npy", werner_measurements)
                    GC.gc()
