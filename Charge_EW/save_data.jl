...
## -------------- Define System ----------------
#Get crash for res_qd = 1, qn = 2 and seed = 3
...
function save_data()
    ...
    #labels_train = vcat([-1 for i in 1:ent_measurements_train.size[1]], [1 for i in 1:sep_measurements_train.size[1]])
    #measurements_train = vcat(hcat(ent_measurements_train), hcat(sep_measurements_train))
    
    #labels_test = vcat([-1 for i in 1:ent_measurements_test.size[1]], [1 for i in 1:sep_measurements_test.size[1]])
