function generate_spin_measurments(ent_state_types, noise_level_min, nbr_ent_states, nbr_sep_states)
    main_sys_qd = 2
    res_qd = 0
    d, d_main, dA_main, dB_main, d_res = total_basis(main_sys_qd, res_qd)

    sep_measurments = measure_sep_states(d_main, dA_main, dB_main, nbr_sep_states)
    ent_measurments = measure_ent_states(d_main, dA_main, dB_main, nbr_ent_states, ent_state_types, noise_level_min)

    return sep_measurments, ent_measurments
end

function measurments(state_list, d_main, dA_main, dB_main)
...
function measure_sep_states(d_main, dA_main, dB_main, nbr_sep_states)
    ...
    sx_list_sep, sy_list_sep, sz_list_sep = measurments(rand_sep_states, d_main, dA_main, dB_main)
...
function measure_ent_states(d_main, dA_main, dB_main, nbr_ent_states, types, noise_level_min)
    ...
    sx_list_ent, sy_list_ent, sz_list_ent = measurments(ent_states, d_main, dA_main, dB_main)
